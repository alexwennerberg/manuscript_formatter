...
TEXT_EXTENSION = ".md" #distinguish between the various markdown extensions

def main():
	print('hello world')

def compile_story_word(name): #doesn't work currently
...
def compile_story_latex(name):
	...
	subprocess.check_call(['pandoc', '--from=markdown_github+yaml_metadata_block',
		'-t', 'latex', name,
		'--latex-engine=xelatex', '--template=story.tex', 
...
def create_story(story_name):
	if os.path.exists(story_name):
		...
		notes_file = story_name + "/" + "notes" + TEXT_EXTENSION
		story_file = story_name + "/" + story_name + TEXT_EXTENSION
		f = open(story_file, 'w')
		open(notes_file, 'w')
		f.write('---' + '\n' + 
				'title: ' + story_name + '\n' +
...
def compile_all_stories():
	...
	for directory_name, subdirectory_list, file_list in os.walk(root_directory):
		for file_name in file_list:
			if root_directory + "/" + file_name == directory_name + TEXT_EXTENSION:
